...
#将数据分词为betch_size为512的长度，不足512的padding填充，然后用掩码将填充的无意义部分遮盖
#所有神经网络模型都只接受batch长度的输入，不足长的也会被打包成batch
def forward(*, model, input_ids, attention_mask, batch_size=512):
    logits = []
    for i in range(0, input_ids.shape[0], batch_size):#0-batch_size,步长
...
def load_model_and_tokenizer(model_path, tokenizer_path=None, device='cuda:0', **kwargs):
    #AutoModelForCausalLM是transformer的模型识别模块
    ...
    #AutoTokenizer是transformer的分词识别模块，取用大模型分词器
    tokenizer = AutoTokenizer.from_pretrained(
        tokenizer_path,
        trust_remote_code=True,
        use_fast=False#transformer的下载路径，如果为true从huggingface官网下，不然就是本地
    )
    #微调分词器
    if 'oasst-sft-6-llama-30b' in tokenizer_path:
        tokenizer.bos_token_id = 1#句首标记为1
        tokenizer.unk_token_id = 0
    if 'guanaco' in tokenizer_path:
        tokenizer.eos_token_id = 2
        tokenizer.unk_token_id = 0
    if 'llama-2' in tokenizer_path:
        tokenizer.pad_token = tokenizer.unk_token#pad。token填充所有token到max长度，将填充标记为unk_token不会被读取
        tokenizer.padding_side = 'left'#padding_side限定填充方向向左
    if 'falcon' in tokenizer_path:
        tokenizer.padding_side = 'left'
    if not tokenizer.pad_token:
        tokenizer.pad_token = tokenizer.eos_token

    return model, tokenizer#制作完成，返回分词器
...
def apply_crossover_and_mutation(selected_data, crossover_probability=0.5, num_points=3, mutation_rate=0.01,
    ...
    for i in range(0, len(selected_data), 2):#从0开始，在len(selected_data)中选择两个父本
        parent1 = selected_data[i]
        parent2 = selected_data[i + 1] if (i + 1) < len(selected_data) else selected_data[0]
        #执行交叉，如果小于交叉率，直接将父本形状输入子代，否则进行5点交叉
...
def crossover(str1, str2, num_points):
    ...
    for para1, para2 in zip(paragraphs1, paragraphs2):
        max_swaps = min(len(para1), len(para2)) - 1#根据句子长度确定最大交换次数
        num_swaps = min(num_points, max_swaps)

        swap_indices = sorted(random.sample(range(1, max_swaps + 1), num_swaps))#标记swap交叉次数

        new_para1, new_para2 = [], []
        last_swap = 0
        #第一级交叉，句子上的词语交叉
        for swap in swap_indices:#根据交换次数swap进行交换
            if random.choice([True, False]):#如果true，交换，并记录交换的位置，直至完成swap次
...
def apply_gpt_mutation(offspring, mutation_rate=0.01, API_key=None, reference=None, if_api=True):
    #突变可分为两种，如果有GPT的API，就用GPT否则用同义词替换
    if if_api:#当API存在
...
def apply_init_gpt_mutation(offspring, mutation_rate=0.01, API_key=None, if_api=True):
    for i in tqdm(range(len(offspring)), desc='initializing...'):#用tqdm显示进度条
        if if_api:
            if random.random() < mutation_rate:
                offspring[i] = gpt_mutate(offspring[i], API_key)
        else:
...
def replace_with_synonyms(sentence, num=10):
    ...
    stop_words = set(stopwords.words('english'))#调用停用词表
    words = nltk.word_tokenize(sentence)#用nltk去停用词
    uncommon_words = [word for word in words if word.lower() not in stop_words and word.lower() not in T]
    selected_words = random.sample(uncommon_words, min(num, len(uncommon_words)))#选择不常用词进行替换，用min()限制次数
...
def construct_momentum_word_dict(word_dict, control_suffixs, score_list, topk=-1):
    ...
    for prefix, score in zip(control_suffixs, score_list):#建表
        words = set(
    ...
    if topk == -1:
        topk_word_dict = dict(list(sorted_word_dict.items()))#返回词典
    else:
        topk_word_dict = dict(list(sorted_word_dict.items())[:topk])#返回前topk个
...
def get_score_autodan(tokenizer, conv_template, instruction, target, model, device, test_controls=None, crit=None,dis=None):
    ...
    # Find the maximum length of input_ids in the list找最大长度
